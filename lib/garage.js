...
function autoClose(time) {
  ...
  console.log('Closing garage in %dms', time);
  setTimeout(function () {
    if (module.exports.status === 'closed') {
      console.log('Garage closed... stoping autoClose');
      autoCloseInProgress = false;
      return;
    }
    console.log('Closing garage via autoClose.');
    module.exports.relay();
    console.log('Making sure it\'s closed in %dms', time)
    autoClose(time);
  }, time);
}
...
function read() {
  gpio.read(7, function (err, value) {
    ...
    if (status !== module.exports.status) {
      console.log('The garage is now %s', status);
      if (module.exports.callback) {
        request({
          ...
          if (err) {
            return console.error('Request to %s failed', module.exports.callback);
          }
          console.log('SmartThings updated successfuly.');
        });
      }
    }
